...
| **Q1_0 binary** (`block_q1_0`, 128/block) | 1-bit {-1,+1} + fp16 block scale | As of build `3f7a40a52` the decode dot uses the 2c-1 identity (bit-spread codes + dp4a, -sum(u) folded through the q8_1 stored sum) and binary now decodes **faster** than ternary, as its byte ratio says it should: Bonsai-**27B** 60.5 t/s vs Q2_0's 49.7 kernel-only, 62.2 vs 51.1 in the appliance (dedup default; rows/block re-swept to 6 for the leaner kernel). (Earlier builds ran a per-bit select chain and measured slower than Q2_0 -- 8B 150 vs 164 t/s -- which was a kernel artifact, not a property of the format.) |
...
| **Activation quant dedup + verify-dedup** (`GGML_HIP_DEDUP_MMVQ_QUANT`) | Sibling matmuls reading the same activation tensor share one `quantize_row_q8_1` launch instead of recomputing byte-identical output; `_BATCH` extends it across the speculative-decode verify pass | Takes Q2_0 MTP decode from a raw ~51 t/s to **67.18 t/s = 87% of the 76.9 t/s roofline for that model**. Lossless, verified across Q2_0/Q4_0/Q4_K/Q5_K/Q6_K/Q8_0/F8E4M3/IU4. **The appliance image defaults BOTH flags ON** (entrypoint, since build `3f7a40a52`); disable with `-e GGML_HIP_DEDUP_MMVQ_QUANT=0` — the ggml check is presence-based, so the entrypoint translates `=0` into an unset. Outside the container the library default is still opt-in. |
