...
| **Q1_0 binary** (`block_q1_0`, 128/block) | 1-bit {-1,+1} + fp16 block scale | As of build `3f7a40a52` the decode dot uses the 2c-1 identity (bit-spread codes + dp4a, -sum(u) folded through the q8_1 stored sum) and binary now decodes **faster** than ternary, as its byte ratio says it should: Bonsai-**27B** 56.4 t/s vs Q2_0's 49.6 kernel-only, 58.3 vs 51.1 with dedup. (Earlier builds ran a per-bit select chain and measured slower than Q2_0 -- 8B 150 vs 164 t/s -- which was a kernel artifact, not a property of the format.) |
